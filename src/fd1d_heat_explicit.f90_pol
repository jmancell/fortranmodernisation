...
! get the CFL coefficient
                ...
                IF (0.5E+00_dp<=cfl) THEN
                                WRITE (*, '(a)') ' '
                                WRITE (*, '(a)') &
                                  'FD1D_HEAT_EXPLICIT_CFL - Fatal error!'
...
! the main time integration loop 
                DO j = 2, t_num
                                CALL fd1d_heat_explicit(x_num, x, t(j-1), dt, &
                                  cfl, h, h_new)
...
! set the boundary conditions again
                ...
                SUBROUTINE r8mat_write(output_filename, m, n, table)
                                ...
                                output_unit_id = 10
                                OPEN (UNIT=output_unit_id, &
                                  FILE=output_filename, STATUS='replace')

                                WRITE (string, '(a1,i8,a1,i8,a1,i8,a1)') '(', &
                                  m, 'g', 24, '.', 16, ')'

                                DO j = 1, n
                                                WRITE (output_unit_id, string) &
                                                  table(1:m, j)
                                END DO
